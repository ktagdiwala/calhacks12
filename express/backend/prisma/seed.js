...
async function main() {
  ...
  const csTags = [
    {
      ...
      userId: users["alice_cs"].id,
    },
    {
      name: "Database Management Systems",
      description: "RDBMS concepts, SQL, indexing",
      userId: users["alice_cs"].id,
    },
    {
    ...
    {
      name: "Algorithms",
      description: "Complexity, sorting, graphs",
      userId: users["alice_cs"].id,
    },
  ];
  ...
  const chemTags = [
    {
      name: "Thermodynamics",
      description: "Energy, enthalpy, entropy",
      userId: users["bob_chem"].id,
    },
    {
      name: "Transport Phenomena",
      description: "Momentum, heat, mass transfer",
      userId: users["bob_chem"].id,
    },
    {
      name: "Chemical Reaction Engineering",
      description: "Kinetics and reactor design",
      userId: users["bob_chem"].id,
    },
    {
      name: "Process Control",
      description: "Control systems and PID",
      userId: users["bob_chem"].id,
    },
  ];
  ...
  const hobbyTags = [
    {
      name: "Japanese",
      description: "Language fundamentals: vocabulary, grammar, kanji",
      userId: users["charlie_learner"].id,
    },
    {
      name: "Music (Guitar)",
      description: "Music theory applied to guitar: chords, scales, rhythm",
      userId: users["charlie_learner"].id,
    },
    {
      name: "Fitness",
      description:
        "Working out principles: strength training, hypertrophy, recovery",
      userId: users["charlie_learner"].id,
    },
  ];
  ...
  for (const t of allTagsList) {
    const tag = await prisma.tag.upsert({
      where: { name: t.name },
      update: { description: t.description, userId: t.userId },
      create: { name: t.name, description: t.description, userId: t.userId },
    });
    tags[tag.name] = tag;
  }
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Software Design", [
      {
        question:
        ...
        type: "FILL_IN_BLANK",
        explanation:
          "The Iterator pattern provides a way to access elements of a collection sequentially without exposing its internal structure. This is commonly used in Java Collections and other frameworks to traverse different types of collections uniformly.",
      },
      {
        question:
          "Which pattern would you use to ensure a class has only one instance and provide a global point of access to it?",
        correctAnswer: "Singleton",
        type: "FILL_IN_BLANK",
        explanation:
          "The Singleton pattern restricts instantiation of a class to one object and provides a global point of access. Common uses include configuration managers, connection pools, and thread pools where exactly one instance is needed.",
      },
      {
        question:
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Database Management Systems", [
      {
        question:
      ...
      {
        question:
          "What is the time complexity of a B-Tree search in terms of number of disk reads?",
  ...
  // Operating Systems
  createdQuestions.push(
    ...(await createQuestionsForTag("Operating Systems", [
      {
        question:
          "What scheduling algorithm selects the process with the smallest next CPU burst?",
        correctAnswer: "Shortest Job First",
        type: "FILL_IN_BLANK",
        explanation:
          "Shortest Job First (SJF) minimizes average waiting time by selecting the process with the shortest expected CPU burst time next. While optimal for minimizing average wait time, it can lead to starvation of longer processes.",
      },
      {
      ...
      {
        question:
          "Calculate the effective access time if memory access = 100ns and TLB hit ratio is 90% with TLB access = 10ns (assume no page fault).",
        correctAnswer:
          "100 + 0.9*10? Actually compute: effective = 0.9*(10+100) + 0.1*(10+100) = 110",
        type: "FILL_IN_BLANK",
        explanation:
          "The effective access time calculation considers both TLB and memory access times. With 90% TLB hit ratio: we always do memory access (100ns) plus TLB access (10ns). So total is always (10+100)ns = 110ns regardless of hit/miss as we ignore page faults.",
      },
    ]))
  );
  ...
  // Algorithms
  createdQuestions.push(
    ...(await createQuestionsForTag("Algorithms", [
      {
      ...
      {
        question:
          "Which algorithm is used to find shortest path in graphs with non-negative weights?",
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Thermodynamics", [
      {
      ...
      {
        question:
          "Calculate the work done by an ideal gas in an isothermal expansion from V1 to V2 at temperature T (R is gas constant).",
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Transport Phenomena", [
      {
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Chemical Reaction Engineering", [
      {
        question:
          "For a first-order irreversible reaction A -> products, write the concentration vs time relation.",
        correctAnswer: "C = C0 * exp(-k*t)",
        type: "FILL_IN_BLANK",
      },
      {
        question:
          "If a plug flow reactor has volume V and inlet flow rate F (L/s) and first-order rate constant k, the conversion X is given by?",
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Process Control", [
      {
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Japanese", [
      {
      ...
      {
        question:
          'Explain how particles like "は (wa)" and "が (ga)" differ in marking the subject/topic of a sentence.',
        correctAnswer:
          "Particles mark topic vs subject: は marks the topic or contrast, が marks the subject or new information.",
        type: "EXPLAIN_PROMPT",
      },
    ]))
  );
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Music (Guitar)", [
      {
        question:
          "What is the interval distance in semitones of a perfect fifth?",
        correctAnswer: "7",
        type: "FILL_IN_BLANK",
      },
      {
      ...
      {
        question:
          "Explain the purpose of palm muting on guitar and when it is commonly used.",
        correctAnswer:
          "Palm muting reduces sustain/volume and adds percussive attack; commonly used in rock/metal rhythm to create tight chugging.",
        type: "EXPLAIN_PROMPT",
      },
    ]))
  );
  ...
  createdQuestions.push(
    ...(await createQuestionsForTag("Fitness", [
      {
      ...
      {
        question:
          "Explain the importance of progressive overload in a training program.",
        correctAnswer:
          "Progressive overload gradually increases stress on muscles (weight, reps, volume) to drive adaptation and continued strength/muscle gains.",
        type: "EXPLAIN_PROMPT",
      },
    ]))
  );
  ...
  for (const [tagName, infos] of Object.entries(sampleFlashcardsByTag)) {
    ...
    for (const info of infos) {
      const exists = await prisma.flashcard.findFirst({
        where: { information: info, tagId: tag.id },
      });
      if (!exists) {
  ...
  const userInputsByUser = {
    alice_cs: [
      {
        description: "Course notes - Software Design",
        content: "Observer, Strategy; remember to review SOLID examples.",
      },
    ],
    bob_chem: [
      {
        ...
        content:
          "Review thermodynamics, reaction kinetics, and process control summary.",
      },
    ],
    charlie_learner: [
      {
        description: "Practice log",
        content:
          "Guitar: pentatonic practice, chord changes. Japanese: 10 vocab words/day.",
      },
    ],
  };
  ...
  for (const [username, inputs] of Object.entries(userInputsByUser)) {
    ...
    for (const ui of inputs) {
      const exists = await prisma.userInput.findFirst({
        where: { userId: user.id, description: ui.description },
      });
      if (!exists) {
  ...
  for (const username of Object.keys(users)) {
    ...
    for (const tagName of Object.keys(tags)) {
      const tag = tags[tagName];
      const tagQuestions = allQuestions
        .filter((q) => q.tagId === tag.id)
        .slice(0, 2);
