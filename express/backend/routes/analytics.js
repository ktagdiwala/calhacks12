...
router.get('/weekly-stats/:userId', async (req, res) => {
  try {
    ...
    if (confidenceLogs.length > 0) {
      const totalDistance = confidenceLogs.reduce((acc, log) => acc + (log.distanceUntilNextDate || 0), 0);
      const avgDistance = totalDistance / confidenceLogs.length;
      // Convert average distance (in days) to a 0-4 scale
      averageConfidence = Math.min(4, Math.round((4 * avgDistance / 7) * 10) / 10);
    }
...
router.get('/topic-progress/:userId', async (req, res) => {
  try {
    ...
    const topicProgress = await Promise.all(tags.map(async (tag) => {
      ...
      const recentLogs = logs.filter(log => {
        ...
        const daysDiff = (Date.now() - logDate.getTime()) / (1000 * 3600 * 24);
        return daysDiff <= 7;
      });
      ...
      const avgConfidence = logs.length > 0 
        ? Math.min(25, (logs.reduce((sum, log) => sum + log.distanceUntilNextDate, 0) / logs.length) * 5)
      ...
      for (let i = 0; i < logs.length; i++) {
        ...
        if ((today.getTime() - logDate.getTime()) / (1000 * 3600 * 24) <= i + 1) {
          streak++;
        } else {
          break;
        }
